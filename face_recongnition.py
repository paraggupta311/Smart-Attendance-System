...
class Face_Recognition:
    def __init__(self,root):
        ...
        f_lbl.place(x=0,y=50,width=600,height=625)


        img_bottom=Image.open(r"college_images\large.jpg")
        img_bottom=img_bottom.resize((900,650),Image.ANTIALIAS)
        self.photoimg_bottom=ImageTk.PhotoImage(img_bottom)

        f_lbl=Label(self.root,image=self.photoimg_bottom)
        f_lbl.place(x=600,y=50,width=820,height=625)
        ...
        b1_1.place(x=300,y=560,width=200,height=40)


    #attendence 
    def mark_attendence(Self,i,r,n,d):
        with open("parag.csv","r+",newline="\n") as f:
            myDataList=f.readlines()
            name_list=[]
            for line in myDataList:
                entry=line.split((","))
                name_list.append(entry[0])
            if((i not in name_list) and (i not in name_list) and (r not in name_list) and (d not in name_list)):
                now=datetime.now()

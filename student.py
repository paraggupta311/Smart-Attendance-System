...
class Student:
    def __init__(self,root):
        ...
        #1st img
        img=Image.open(r"C:\Users\hp\Desktop\Face recongnition\college_images\face-recognition.png")
        ...
        # 2nd img
        img1=Image.open(r"C:\Users\hp\Desktop\Face recongnition\college_images\smart-attendance.jpg")
        ...
        # third img
        img2=Image.open(r"C:\Users\hp\Desktop\Face recongnition\college_images\rd.jpg")
        img2=img2.resize((500,130),Image.ANTIALIAS)
        self.photoimg2=ImageTk.PhotoImage(img2)

        f_lbl=Label(self.root,image=self.photoimg2)
        f_lbl.place(x=980,y=0,width=380,height=130)

        #bg img
        img3=Image.open(r"C:\Users\hp\Desktop\Face recongnition\college_images\wp2551980.jpg")
        ...
        title_lbl.place(x=0,y=-5,width="1430",height="30")

        main_frame=Frame(bg_img,bd=2,bg="white")
        main_frame.place(x=15,y=20,width=1320,height=600)

        #left lable frame
        Left_frame=LabelFrame(main_frame,bd=2,bg="white",relief=RIDGE,text="Student Details",font=("times new roman",12,"bold"))
        Left_frame.place(x=10,y=10,width=690,height=530)

        img_left=Image.open(r"C:\Users\hp\Desktop\Face recongnition\college_images\Adobe.jpeg")
        ...
        dep_combo["values"]=("Select Department","Computer","IT","Civil","Mechanical","Electronics")
        dep_combo.current(0)
        ...
        class_Student_frame.place(x=5,y=260,width=680,height=240)
        ...
        btn_frame.place(x=0,y=130,width=670,height=40)
        ...
        btn_frame1.place(x=0,y=175,width=670,height=35)

        take_photo_btn=Button(btn_frame1,command=self.generate_dataset,text="Take Photo Sample",width=32,font=("times new roman",13,"bold"),bg="blue",fg="white")
        ...
        Right_frame.place(x=710,y=10,width=590,height=530)


        img_right=Image.open(r"C:\Users\hp\Desktop\Face recongnition\college_images\student.jpg")
        ...
        search_combo=ttk.Combobox(Search_frame,textvariable=self.search_by,font=("times new roman",12,"bold"),state="readonly",width=12)
        search_combo["values"]=("Select" ,"RollNo","Name")
        search_combo.current(0)
        search_combo.grid(row=0,column=1,padx=2,pady=10,sticky=W)

        search_entry=ttk.Entry(Search_frame,textvariable=self.search_txt,width=12,font=("times new roman",12,"bold"))
        ...
        self.student_table=ttk.Treeview(table_frame,column=("dep","course","year","sem","id","name","div","rollno","gender","dob"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
    ...
    # add functions
    def add_data(self):
        if self.var_dep.get()=="Select Department" or self.var_stud_name.get()=="" or self.var_stu_id.get()=="":
            messagebox.showerror("Error","All Fields are required",parent=self.root)
        else:
            try:
                conn = mysql.connector.connect(host="localhost", user="root", password="admin", database="face_recongnization", auth_plugin='mysql_native_password')
                ...
                self.fetch_data() 
                conn.close()
                messagebox.showinfo("Success","Students details have been added succesfully",parent=self.root)
        ...
        if self.var_dep.get()=="Select Department" or self.var_stud_name.get()=="" or self.var_stu_id.get()=="":
            ...
            try:
                ...
                if update>0:
                    conn = mysql.connector.connect(host="localhost", user="root", password="admin", database="face_recongnization", auth_plugin='mysql_native_password')
                    my_cursor=conn.cursor()
                    my_cursor.execute("update student set dep=%s, course=%s, Year=%s, Semester=%s, Name=%s, Division=%s, RollNo=%s, Gender=%s, DOB=%s ,PhotoSample=%s where Student_id=%s",(
                                                                                                                                                            self.var_dep.get(),
                                                                                                                                                            self.var_course.get(),
                                                                                                                                                            self.var_year.get(),
                                                                                                                                                            self.var_semester.get(),
                    ...
                    if not update:
                        return
                messagebox.showinfo("Success","Student deatils updated successfully",parent=self.root)
                conn.commit()
                self.fetch_data()
                conn.close()
            except Exception as es:
    ...
    def delete_data(self):
        if self.var_stu_id.get()=="":
            ...
            try:
                delete=messagebox.askyesno("Student delete page","Do you want to delte this student ",parent=self.root )
                if delete>0:
                    conn = mysql.connector.connect(
                    host="localhost",
                    user="root",
                    password="admin",
                    database="face_recongnization",
    ...
    def generate_dataset(self):
        if(
            ...
            or self.var_stud_name.get() == ""
            or self.var_stu_id.get() == ""
        ):
            messagebox.showerror("Error", "All Fields are required", parent=self.root)
        else:
            try:
                ...
                for x in myresult:
                    id + 1
                query = "UPDATE student SET dep=%s, course=%s, Year=%s, Semester=%s, Name=%s, Division=%s, RollNo=%s, Gender=%s, DOB=%s, PhotoSample=%s WHERE Student_id=%s"
                values = (
                    self.var_dep.get(),
                    self.var_course.get(),
                    self.var_year.get(),
